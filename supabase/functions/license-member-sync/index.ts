...
Deno.serve(async req=>{
  ...
  if(body.action==="pull"){
    ...
    const{data:ents,error:entError}=await db.from("license_entitlements").select("user_id,product_code,status,starts_at,expires_at,allowed_subjects,updated_at");if(entError)return out({error:entError.message},500);
  ...
  if(body.action==="push"){
    ...
    for(const row of rows){try{let userId=String(row.user_id||"");if(!userId&&row.email){const{data}=await db.from("member_profiles").select("id").ilike("email",String(row.email)).maybeSingle();userId=data?.id||""}if(!userId)throw Error("Member not found");let wanted=Array.isArray(row.products)?row.products.map(String):[];if(wanted.includes("ALL"))wanted=[...codes];wanted=[...new Set(wanted.filter(x=>codes.has(x)))];const status=validStatus.has(String(row.status||"").toLowerCase())?String(row.status).toLowerCase():"pending",starts=row.starts_at||new Date().toISOString(),expires=row.expires_at||null;if(status==="active"&&wanted.length){const{error:confirmError}=await db.auth.admin.updateUserById(userId,{email_confirm:true});if(confirmError)throw confirmError}const rowSubjects=row.subjects&&typeof row.subjects==="object"&&!Array.isArray(row.subjects)?row.subjects:{};for(const code of wanted){const raw=rowSubjects[code];const subjects=Array.isArray(raw)?[...new Set(raw.map((s:unknown)=>String(s).trim()).filter((s:string)=>s&&s.length<=120))]:[];const{error}=await db.from("license_entitlements").upsert({user_id:userId,product_code:code,status,starts_at:starts,expires_at:expires,allowed_subjects:subjects,metadata:{payment:String(row.payment||""),note:String(row.note||""),source:"google_sheet"},updated_at:new Date().toISOString()},{onConflict:"user_id,product_code"});if(error)throw error}const{data:existing,error:listError}=await db.from("license_entitlements").select("product_code").eq("user_id",userId).eq("status","active");if(listError)throw listError;const removed=(existing||[]).map(e=>e.product_code).filter(code=>!wanted.includes(code));if(removed.length){const{error}=await db.from("license_entitlements").update({status:"inactive",updated_at:new Date().toISOString()}).eq("user_id",userId).in("product_code",removed);if(error)throw error}results.push({user_id:userId,ok:true,products:wanted,status})}catch(error){results.push({user_id:row.user_id||"",email:row.email||"",ok:false,error:error instanceof Error?error.message:String(error)})}}
    return out({results});
  }
